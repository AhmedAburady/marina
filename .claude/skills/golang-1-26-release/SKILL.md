---
name: golang-1-26-release
description: "Golang 1.26 release notes - use when you need to know what changed in Go 1.26 and how to use the new features."
user-invocable: true
---

# Go 1.26 Release Notes

## Introduction to Go 1.26 [¶](https://go.dev/doc/go1.26\#introduction)

The latest Go release, version 1.26, arrives in [February 2026](https://go.dev/doc/devel/release#go1.26.0), six months after [Go 1.25](https://go.dev/doc/go1.25).
Most of its changes are in the implementation of the toolchain, runtime, and libraries.
As always, the release maintains the Go 1 promise of compatibility.
We expect almost all Go programs to continue to compile and run as before.

## Changes to the language [¶](https://go.dev/doc/go1.26\#language)
...
## Tools [¶](https://go.dev/doc/go1.26\#tools)

### Go command [¶](https://go.dev/doc/go1.26\#go-command)

The venerable `go fix` command has been completely revamped and is now the home
of Go’s _modernizers_. It provides a dependable, push-button way to update Go
...
any issues with a fix performed by `go fix`, please [report it](https://go.dev/issue/new).
...
### Pprof [¶](https://go.dev/doc/go1.26\#pprof)
...
### New garbage collector [¶](https://go.dev/doc/go1.26\#new-garbage-collector)
...
performance or behavior, please [file an issue](https://go.dev/issue/new).

### Faster cgo calls [¶](https://go.dev/doc/go1.26\#faster-cgo-calls)

The baseline runtime overhead of cgo calls has been reduced by ~30%.

### Heap base address randomization [¶](https://go.dev/doc/go1.26\#heap-base-address-randomization)
...
### Experimental goroutine leak profile [¶](https://go.dev/doc/go1.26\#goroutineleak-profiles)

A new profile type that reports leaked goroutines is now available as an
experiment.
The new profile type, named `goroutineleak` in the
[`runtime/pprof`](https://go.dev/pkg/runtime/pprof) package, may be enabled by setting
`GOEXPERIMENT=goroutineleakprofile` at build time.
Enabling the experiment also makes the profile available as a
[`net/http/pprof`](https://go.dev/pkg/net/http/pprof) endpoint,
`/debug/pprof/goroutineleak`.

A _leaked_ goroutine is a goroutine blocked on some concurrency primitive
(channels, [`sync.Mutex`](https://go.dev/pkg/sync#Mutex), [`sync.Cond`](https://go.dev/pkg/sync#Cond), etc) that
...
any runnable goroutine or any goroutine that _those_ could unblock, then P
...
If you encounter issues with this optimization, please [file an issue](https://go.dev/issue/new).

## Linker [¶](https://go.dev/doc/go1.26\#linker)
...
## Standard library [¶](https://go.dev/doc/go1.26\#library)

### New crypto/hpke package [¶](https://go.dev/doc/go1.26\#new-cryptohpke-package)

The new [`crypto/hpke`](https://go.dev/pkg/crypto/hpke) package implements Hybrid Public Key Encryption
(HPKE) as specified in [RFC 9180](https://rfc-editor.org/rfc/rfc9180.html), including support for post-quantum
hybrid KEMs.

### New experimental simd/archsimd package [¶](https://go.dev/doc/go1.26\#simd)

Go 1.26 introduces a new experimental [`simd/archsimd`](https://go.dev/pkg/simd/archsimd/)
...
[`Int8x16`](https://go.dev/pkg/simd/archsimd#Int8x16) and
[`Float64x8`](https://go.dev/pkg/simd/archsimd#Float64x8), with operations such as
[`Int8x16.Add`](https://go.dev/pkg/simd/archsimd#Int8x16.Add).
...
See the [package documentation](https://go.dev/pkg/simd/archsimd) and the [proposal issue](https://go.dev/issue/73787) for more details.

### New experimental runtime/secret package [¶](https://go.dev/doc/go1.26\#new-experimental-runtimesecret-package)

The new [`runtime/secret`](https://go.dev/pkg/runtime/secret) package is available as an experiment,
...
### Minor changes to the library [¶](https://go.dev/doc/go1.26\#minor_library_changes)

#### [`bytes`](https://go.dev/pkg/bytes/) [¶](https://go.dev/doc/go1.26\#bytespkgbytes)

The new [`Buffer.Peek`](https://go.dev/pkg/bytes#Buffer.Peek) method returns the next n bytes from the buffer without
advancing it.

#### [`crypto`](https://go.dev/pkg/crypto/) [¶](https://go.dev/doc/go1.26\#cryptopkgcrypto)

The new [`Encapsulator`](https://go.dev/pkg/crypto#Encapsulator) and [`Decapsulator`](https://go.dev/pkg/crypto#Decapsulator) interfaces allow accepting abstract
KEM encapsulation or decapsulation keys.

#### [`crypto/dsa`](https://go.dev/pkg/crypto/dsa/) [¶](https://go.dev/doc/go1.26\#cryptodsapkgcryptodsa)

The random parameter to [`GenerateKey`](https://go.dev/pkg/crypto/dsa#GenerateKey) is now ignored.
Instead, it now always uses a secure source of cryptographically random bytes.
For deterministic testing, use the new [`testing/cryptotest.SetGlobalRandom`](https://go.dev/pkg/testing/cryptotest#SetGlobalRandom) function.
...
The random parameter to [`Curve.GenerateKey`](https://go.dev/pkg/crypto/ecdh#Curve.GenerateKey) is now ignored.
Instead, it now always uses a secure source of cryptographically random bytes.
For deterministic testing, use the new [`testing/cryptotest.SetGlobalRandom`](https://go.dev/pkg/testing/cryptotest#SetGlobalRandom) function.
The new GODEBUG setting `cryptocustomrand=1` temporarily restores the old behavior.

The new [`KeyExchanger`](https://go.dev/pkg/crypto/ecdh#KeyExchanger) interface, implemented by [`PrivateKey`](https://go.dev/pkg/crypto/ecdh#PrivateKey), makes it possible
to accept abstract ECDH private keys, e.g. those implemented in hardware.

#### [`crypto/ecdsa`](https://go.dev/pkg/crypto/ecdsa/) [¶](https://go.dev/doc/go1.26\#cryptoecdsapkgcryptoecdsa)

The `big.Int` fields of [`PublicKey`](https://go.dev/pkg/crypto/ecdsa#PublicKey) and [`PrivateKey`](https://go.dev/pkg/crypto/ecdsa#PrivateKey) are now deprecated.

The random parameter to [`GenerateKey`](https://go.dev/pkg/crypto/ecdsa#GenerateKey), [`SignASN1`](https://go.dev/pkg/crypto/ecdsa#SignASN1), [`Sign`](https://go.dev/pkg/crypto/ecdsa#Sign), and [`PrivateKey.Sign`](https://go.dev/pkg/crypto/ecdsa#PrivateKey.Sign) is now ignored.
Instead, they now always use a secure source of cryptographically random bytes.
For deterministic testing, use the new [`testing/cryptotest.SetGlobalRandom`](https://go.dev/pkg/testing/cryptotest#SetGlobalRandom) function.
The new GODEBUG setting `cryptocustomrand=1` temporarily restores the old behavior.

#### [`crypto/ed25519`](https://go.dev/pkg/crypto/ed25519/) [¶](https://go.dev/doc/go1.26\#cryptoed25519pkgcryptoed25519)

If the random parameter to [`GenerateKey`](https://go.dev/pkg/crypto/ed25519#GenerateKey) is nil, GenerateKey now always uses a
secure source of cryptographically random bytes, instead of [`crypto/rand.Reader`](https://go.dev/pkg/crypto/rand#Reader)
(which could have been overridden). The new GODEBUG setting `cryptocustomrand=1`
temporarily restores the old behavior.

#### [`crypto/fips140`](https://go.dev/pkg/crypto/fips140/) [¶](https://go.dev/doc/go1.26\#cryptofips140pkgcryptofips140)

[FIPS 140-3 Go Cryptographic Module](https://go.dev/doc/security/fips140) v1.26.0 includes changes made to the `crypto/internal/fips140/...` packages up to this release, and can now be selected with GOFIPS140.

The new [`WithoutEnforcement`](https://go.dev/pkg/crypto/fips140#WithoutEnforcement) and [`Enforced`](https://go.dev/pkg/crypto/fips140#Enforced) functions now allow running
in `GODEBUG=fips140=only` mode while selectively disabling the strict FIPS 140-3 checks.

[`Version`](https://go.dev/pkg/crypto/fips140#Version) returns the resolved FIPS 140-3 Go Cryptographic Module version when building against a frozen module with GOFIPS140.

#### [`crypto/mlkem`](https://go.dev/pkg/crypto/mlkem/) [¶](https://go.dev/doc/go1.26\#cryptomlkempkgcryptomlkem)

The new [`DecapsulationKey768.Encapsulator`](https://go.dev/pkg/crypto/mlkem#DecapsulationKey768.Encapsulator) and
[`DecapsulationKey1024.Encapsulator`](https://go.dev/pkg/crypto/mlkem#DecapsulationKey1024.Encapsulator) methods implement the new
[`crypto.Decapsulator`](https://go.dev/pkg/crypto#Decapsulator) interface.

Encapsulation and decapsultion operations are now approximately 18% faster.

#### [`crypto/mlkem/mlkemtest`](https://go.dev/pkg/crypto/mlkem/mlkemtest/) [¶](https://go.dev/doc/go1.26\#cryptomlkemmlkemtestpkgcryptomlkemmlkemtest)

The new [`crypto/mlkem/mlkemtest`](https://go.dev/pkg/crypto/mlkem/mlkemtest) package exposes the [`Encapsulate768`](https://go.dev/pkg/crypto/mlkem/mlkemtest#Encapsulate768) and
[`Encapsulate1024`](https://go.dev/pkg/crypto/mlkem/mlkemtest#Encapsulate1024) functions which implement derandomized ML-KEM encapsulation,
for use with known-answer tests.

#### [`crypto/rand`](https://go.dev/pkg/crypto/rand/) [¶](https://go.dev/doc/go1.26\#cryptorandpkgcryptorand)

The random parameter to [`Prime`](https://go.dev/pkg/crypto/rand#Prime) is now ignored.
Instead, it now always uses a secure source of cryptographically random bytes.
For deterministic testing, use the new [`testing/cryptotest.SetGlobalRandom`](https://go.dev/pkg/testing/cryptotest#SetGlobalRandom) function.
The new GODEBUG setting `cryptocustomrand=1` temporarily restores the old behavior.

#### [`crypto/rsa`](https://go.dev/pkg/crypto/rsa/) [¶](https://go.dev/doc/go1.26\#cryptorsapkgcryptorsa)

The new [`EncryptOAEPWithOptions`](https://go.dev/pkg/crypto/rsa#EncryptOAEPWithOptions) function allows specifying different hash
functions for OAEP padding and MGF1 mask generation.

The random parameter to [`GenerateKey`](https://go.dev/pkg/crypto/rsa#GenerateKey), [`GenerateMultiPrimeKey`](https://go.dev/pkg/crypto/rsa#GenerateMultiPrimeKey), and [`EncryptPKCS1v15`](https://go.dev/pkg/crypto/rsa#EncryptPKCS1v15) is now ignored.
Instead, they now always use a secure source of cryptographically random bytes.
For deterministic testing, use the new [`testing/cryptotest.SetGlobalRandom`](https://go.dev/pkg/testing/cryptotest#SetGlobalRandom) function.
The new GODEBUG setting `cryptocustomrand=1` temporarily restores the old behavior.

If [`PrivateKey`](https://go.dev/pkg/crypto/rsa#PrivateKey) fields are modified after calling [`PrivateKey.Precompute`](https://go.dev/pkg/crypto/rsa#PrivateKey.Precompute),
[`PrivateKey.Validate`](https://go.dev/pkg/crypto/rsa#PrivateKey.Validate) now fails.

[`PrivateKey.D`](https://go.dev/pkg/crypto/rsa#PrivateKey.D) is now checked for consistency with precomputed values, even if
it is not used.

Unsafe PKCS #1 v1.5 encryption padding (implemented by [`EncryptPKCS1v15`](https://go.dev/pkg/crypto/rsa#EncryptPKCS1v15),
[`DecryptPKCS1v15`](https://go.dev/pkg/crypto/rsa#DecryptPKCS1v15), and [`DecryptPKCS1v15SessionKey`](https://go.dev/pkg/crypto/rsa#DecryptPKCS1v15SessionKey)) is now deprecated.

#### [`crypto/sha3`](https://go.dev/pkg/crypto/sha3/) [¶](https://go.dev/doc/go1.26\#cryptosha3pkgcryptosha3)

The zero value of [`SHA3`](https://go.dev/pkg/crypto/sha3#SHA3) is now a usable SHA3-256 instance, and the zero value of [`SHAKE`](https://go.dev/pkg/crypto/sha3#SHAKE) is now a usable SHAKE256 instance.

#### [`crypto/subtle`](https://go.dev/pkg/crypto/subtle) [¶](https://go.dev/doc/go1.26\#cryptosubtlepkgcryptosubtle)

The [`WithDataIndependentTiming`](https://go.dev/pkg/crypto/subtle#WithDataIndependentTiming)
...
#### [`crypto/tls`](https://go.dev/pkg/crypto/tls/) [¶](https://go.dev/doc/go1.26\#cryptotlspkgcryptotls)

The hybrid [`SecP256r1MLKEM768`](https://go.dev/pkg/crypto/tls#SecP256r1MLKEM768) and [`SecP384r1MLKEM1024`](https://go.dev/pkg/crypto/tls#SecP384r1MLKEM1024) post-quantum key
exchanges are now enabled by default. They can be disabled by setting
[`Config.CurvePreferences`](https://go.dev/pkg/crypto/tls#Config.CurvePreferences) or with the `tlssecpmlkem=0` GODEBUG setting.

The new [`ClientHelloInfo.HelloRetryRequest`](https://go.dev/pkg/crypto/tls#ClientHelloInfo.HelloRetryRequest) field indicates if the ClientHello
was sent in response to a HelloRetryRequest message. The new
[`ConnectionState.HelloRetryRequest`](https://go.dev/pkg/crypto/tls#ConnectionState.HelloRetryRequest) field indicates if the server
sent a HelloRetryRequest, or if the client received a HelloRetryRequest,
depending on connection role.

The [`QUICConn`](https://go.dev/pkg/crypto/tls#QUICConn) type used by QUIC implementations includes a new event
for reporting TLS handshake errors.

If [`Certificate.PrivateKey`](https://go.dev/pkg/crypto/tls#Certificate.PrivateKey) implements [`crypto.MessageSigner`](https://go.dev/pkg/crypto#MessageSigner), its SignMessage
method is used instead of Sign in TLS 1.2 and later.

The following GODEBUG settings introduced in [Go 1.22](https://go.dev/doc/godebug#go-122)
and [Go 1.23](https://go.dev/doc/godebug#go-123) will be removed in the next major Go release.
Starting in Go 1.27, the new behavior will apply regardless of GODEBUG setting or go.mod language version.

- `tlsunsafeekm`: [`ConnectionState.ExportKeyingMaterial`](https://go.dev/pkg/crypto/tls#ConnectionState.ExportKeyingMaterial) will require TLS 1.3 or Extended Master Secret.
...
- `x509keypairleaf`: [`X509KeyPair`](https://go.dev/pkg/crypto/tls#X509KeyPair) and [`LoadX509KeyPair`](https://go.dev/pkg/crypto/tls#LoadX509KeyPair) will always populate the [`Certificate.Leaf`](https://go.dev/pkg/crypto/tls#Certificate.Leaf) field.

#### [`crypto/x509`](https://go.dev/pkg/crypto/x509/) [¶](https://go.dev/doc/go1.26\#cryptox509pkgcryptox509)

The [`ExtKeyUsage`](https://go.dev/pkg/crypto/x509#ExtKeyUsage) and [`KeyUsage`](https://go.dev/pkg/crypto/x509#KeyUsage) types now have `String` methods that return the
corresponding OID names as defined in RFC 5280 and other registries.

The [`ExtKeyUsage`](https://go.dev/pkg/crypto/x509#ExtKeyUsage) type now has an `OID` method that returns the corresponding OID for the EKU.

The new [`OIDFromASN1OID`](https://go.dev/pkg/crypto/x509#OIDFromASN1OID) function allows converting an [`encoding/asn1.ObjectIdentifier`](https://go.dev/pkg/encoding/asn1#ObjectIdentifier) into
an [`OID`](https://go.dev/pkg/crypto/x509#OID).

#### [`debug/elf`](https://go.dev/pkg/debug/elf/) [¶](https://go.dev/doc/go1.26\#debugelfpkgdebugelf)
...
#### [`errors`](https://go.dev/pkg/errors/) [¶](https://go.dev/doc/go1.26\#errorspkgerrors)

The new [`AsType`](https://go.dev/pkg/errors#AsType) function is a generic version of [`As`](https://go.dev/pkg/errors#As). It is type-safe, faster,
and, in most cases, easier to use.

#### [`fmt`](https://go.dev/pkg/fmt/) [¶](https://go.dev/doc/go1.26\#fmtpkgfmt)

For unformatted strings, `fmt.Errorf("x")` now allocates less and generally matches
the allocations for `errors.New("x")`.

#### [`go/ast`](https://go.dev/pkg/go/ast/) [¶](https://go.dev/doc/go1.26\#goastpkggoast)

The new [`ParseDirective`](https://go.dev/pkg/go/ast#ParseDirective) function parses [directive\\
comments](https://go.dev/doc/comment#Syntax), which are comments such as `//go:generate`.
...
The new [`BasicLit.ValueEnd`](https://go.dev/pkg/go/ast#BasicLit.ValueEnd) field records the precise end position of
a literal so that the [`BasicLit.End`](https://go.dev/pkg/go/ast#BasicLit.End) method can now always return the
...
#### [`go/token`](https://go.dev/pkg/go/token/) [¶](https://go.dev/doc/go1.26\#gotokenpkggotoken)

The new [`File.End`](https://go.dev/pkg/go/token#File.End) convenience method returns the file’s end position.

#### [`go/types`](https://go.dev/pkg/go/types/) [¶](https://go.dev/doc/go1.26\#gotypespkggotypes)

The `gotypesalias` GODEBUG setting introduced in [Go 1.22](https://go.dev/doc/godebug#go-122)
will be removed in the next major Go release.
Starting in Go 1.27, the [`go/types`](https://go.dev/pkg/go/types) package will always produce an
[Alias type](https://go.dev/pkg/go/types#Alias) for the representation of [type aliases](https://go.dev/ref/spec#Type_declarations)
regardless of GODEBUG setting or go.mod language version.

#### [`image/jpeg`](https://go.dev/pkg/image/jpeg/) [¶](https://go.dev/doc/go1.26\#imagejpegpkgimagejpeg)
...
#### [`io`](https://go.dev/pkg/io/) [¶](https://go.dev/doc/go1.26\#iopkgio)

[`ReadAll`](https://go.dev/pkg/io#ReadAll) now allocates less intermediate memory and returns a minimally sized
...
#### [`log/slog`](https://go.dev/pkg/log/slog/) [¶](https://go.dev/doc/go1.26\#logslogpkglogslog)

The [`NewMultiHandler`](https://go.dev/pkg/log/slog#NewMultiHandler) function creates a
[`MultiHandler`](https://go.dev/pkg/log/slog#MultiHandler) that invokes all the given Handlers.
...
#### [`net`](https://go.dev/pkg/net/) [¶](https://go.dev/doc/go1.26\#netpkgnet)

The new [`Dialer`](https://go.dev/pkg/net/#Dialer) methods
[`DialIP`](https://go.dev/pkg/net/#Dialer.DialIP),
[`DialTCP`](https://go.dev/pkg/net/#Dialer.DialTCP),
[`DialUDP`](https://go.dev/pkg/net/#Dialer.DialUDP), and
[`DialUnix`](https://go.dev/pkg/net/#Dialer.DialUnix)
permit dialing specific network types with context values.

#### [`net/http`](https://go.dev/pkg/net/http/) [¶](https://go.dev/doc/go1.26\#nethttppkgnethttp)

The new
[`HTTP2Config.StrictMaxConcurrentRequests`](https://go.dev/pkg/net/http#HTTP2Config.StrictMaxConcurrentRequests)
field controls whether a new connection should be opened
if an existing HTTP/2 connection has exceeded its stream limit.

The new [`Transport.NewClientConn`](https://go.dev/pkg/net/http#Transport.NewClientConn) method returns a client connection
to an HTTP server.
Most users should continue to use [`Transport.RoundTrip`](https://go.dev/pkg/net/http#Transport.RoundTrip) to make requests,
...
[`Client`](https://go.dev/pkg/net/http#Client) now uses and sets cookies scoped to URLs with the host portion matching
[`Request.Host`](https://go.dev/pkg/net/http#Request.Host) when available.
Previously, the connection address host was always used.
[`ServeMux`](https://go.dev/pkg/net/http#ServeMux) trailing slash redirects now use HTTP status 307
(Temporary Redirect) instead of 301 (Moved Permanently).

#### [`net/http/httptest`](https://go.dev/pkg/net/http/httptest/) [¶](https://go.dev/doc/go1.26\#nethttphttptestpkgnethttphttptest)

The HTTP client returned by [`Server.Client`](https://go.dev/pkg/net/http/httptest#Server.Client) will now redirect requests for
`example.com` and any subdomains to the server being tested.

#### [`net/http/httputil`](https://go.dev/pkg/net/http/httputil/) [¶](https://go.dev/doc/go1.26\#nethttphttputilpkgnethttphttputil)

The [`ReverseProxy.Director`](https://go.dev/pkg/net/http/httputil#ReverseProxy.Director) configuration field is deprecated
in favor of [`ReverseProxy.Rewrite`](https://go.dev/pkg/net/http/httputil#ReverseProxy.Rewrite).
...
#### [`net/netip`](https://go.dev/pkg/net/netip/) [¶](https://go.dev/doc/go1.26\#netnetippkgnetnetip)

The new [`Prefix.Compare`](https://go.dev/pkg/net/netip#Prefix.Compare) method compares two prefixes.

#### [`net/url`](https://go.dev/pkg/net/url/) [¶](https://go.dev/doc/go1.26\#neturlpkgneturl)

[`Parse`](https://go.dev/pkg/net/url#Parse) now rejects malformed URLs containing colons in the host subcomponent,
...
#### [`os`](https://go.dev/pkg/os/) [¶](https://go.dev/doc/go1.26\#ospkgos)

The new [`Process.WithHandle`](https://go.dev/pkg/os#Process.WithHandle) method provides
access to an internal process handle on supported platforms (pidfd on Linux 5.4
or later, Handle on Windows).

On Windows, the [`OpenFile`](https://go.dev/pkg/os#OpenFile)`flag` parameter can now contain any combination of
...
#### [`os/signal`](https://go.dev/pkg/os/signal/) [¶](https://go.dev/doc/go1.26\#ossignalpkgossignal)

[`NotifyContext`](https://go.dev/pkg/os/signal#NotifyContext) now cancels the returned context with [`context.CancelCauseFunc`](https://go.dev/pkg/context#CancelCauseFunc)
and an error indicating which signal was received.

#### [`reflect`](https://go.dev/pkg/reflect/) [¶](https://go.dev/doc/go1.26\#reflectpkgreflect)

The new methods [`Type.Fields`](https://go.dev/pkg/reflect#Type.Fields),
[`Type.Methods`](https://go.dev/pkg/reflect#Type.Methods),
[`Type.Ins`](https://go.dev/pkg/reflect#Type.Ins)
and [`Type.Outs`](https://go.dev/pkg/reflect#Type.Outs)
...
Similarly, the new methods [`Value.Fields`](https://go.dev/pkg/reflect#Value.Fields)
and [`Value.Methods`](https://go.dev/pkg/reflect#Value.Methods) return iterators over
a value’s fields or methods, respectively.
Each iteration yields the type information ( [`StructField`](https://go.dev/pkg/reflect#StructField) or
[`Method`](https://go.dev/pkg/reflect#Method)) of a field or method,
along with the field or method [`Value`](https://go.dev/pkg/reflect#Value).

#### [`runtime/metrics`](https://go.dev/pkg/runtime/metrics/) [¶](https://go.dev/doc/go1.26\#runtimemetricspkgruntimemetrics)
...
#### [`testing`](https://go.dev/pkg/testing/) [¶](https://go.dev/doc/go1.26\#testingpkgtesting)

The new methods [`T.ArtifactDir`](https://go.dev/pkg/testing#T.ArtifactDir), [`B.ArtifactDir`](https://go.dev/pkg/testing#B.ArtifactDir), and [`F.ArtifactDir`](https://go.dev/pkg/testing#F.ArtifactDir)
...
The [`B.Loop`](https://go.dev/pkg/testing#B.Loop) method no longer prevents inlining in
...
[`B.N`](https://go.dev/pkg/testing#B) style to the new `B.Loop` style with no ill effects.
...
#### [`testing/cryptotest`](https://go.dev/pkg/testing/cryptotest/) [¶](https://go.dev/doc/go1.26\#testingcryptotestpkgtestingcryptotest)

The new [`SetGlobalRandom`](https://go.dev/pkg/testing/cryptotest#SetGlobalRandom) function configures a global, deterministic
...
#### [`time`](https://go.dev/pkg/time/) [¶](https://go.dev/doc/go1.26\#timepkgtime)

The `asynctimerchan` GODEBUG setting introduced in [Go 1.23](https://go.dev/doc/godebug#go-123)
will be removed in the next major Go release.
Starting in Go 1.27, the [`time`](https://go.dev/pkg/time) package will always use unbuffered
(synchronous) channels for timers regardless of GODEBUG setting or go.mod language version.

## Ports [¶](https://go.dev/doc/go1.26\#ports)

### Darwin [¶](https://go.dev/doc/go1.26\#darwin)

Go 1.26 is the last release that will run on macOS 12 Monterey. Go 1.27 will require macOS 13 Ventura or later.

### FreeBSD [¶](https://go.dev/doc/go1.26\#freebsd)

The freebsd/riscv64 port (`GOOS=freebsd GOARCH=riscv64`) has been marked [broken](https://go.dev/wiki/PortingPolicy#broken-ports).
See [issue 76475](https://go.dev/issue/76475) for details.

### Windows [¶](https://go.dev/doc/go1.26\#windows)

As [announced](https://go.dev/doc/go1.25#windows) in the Go 1.25 release notes, the
[broken](https://go.dev/doc/go1.24#windows) 32-bit windows/arm port (`GOOS=windows``GOARCH=arm`) has been removed.

### PowerPC [¶](https://go.dev/doc/go1.26\#powerpc)
...
### RISC-V [¶](https://go.dev/doc/go1.26\#risc-v)
